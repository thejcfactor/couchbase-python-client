...
from new_couchbase.common.auth import CertificateAuthenticator, PasswordAuthenticator
...
from new_couchbase.options import TLSVerifyMode
from couchbase.pycbc_core import (close_connection,
                                  ...
                                  mgmt_operations,
                                  operations)
from new_couchbase.result import (ClusterInfoResult,
                              DiagnosticsResult,
                              PingResult)
from new_couchbase.serializer import DefaultJsonSerializer
from new_couchbase.transcoder import JSONTranscoder

from new_couchbase.api.serializer import SerializerInterface
from new_couchbase.api.transcoder import TranscoderInterface

from new_couchbase.impl.server.options import ValidClusterOptions, ValidDiagnosticOptions

if TYPE_CHECKING:
    from new_couchbase.options import DiagnosticsOptions, PingOptions, ClusterOptions

log = logging.getLogger(__name__)

class ClusterCore(ClusterCoreInterface):
    ...
    def __init__(self,  # noqa: C901
        ...
        timeout_opts = {}
        for key in ValidClusterOptions.get_allowed_option_keys(OptionTypes.ClusterTimeout, use_transform_keys=True):
        ...
        tracing_opts = {}
        for key in ValidClusterOptions.get_allowed_option_keys(OptionTypes.ClusterTracing, use_transform_keys=True):
            if key in cluster_opts:
    ...
    def cluster_info(self) -> ClusterInfoResult:
        """
            `~new_couchbase.result.ClusterInfoResult`: Cluster information.
        """
        return self._cluster_info

    @cluster_info.setter
    def cluster_info(self, value # type: ClusterInfoResult
        ) -> None:
    ...
    def connect_cluster(self, **kwargs):
    ...
    def diagnostics(self,
        ...
        final_args = parse_options(ValidDiagnosticOptions.get_valid_options(OptionTypes.Diagnostics), kwargs, *opts)
        diagnostics_kwargs.update(final_args)
        return diagnostics_operation(**diagnostics_kwargs)

    def get_cluster_info(self, **kwargs # type: Dict[str, Any]
        ) -> ClusterInfoResult:
