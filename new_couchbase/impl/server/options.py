...
from new_couchbase.common.diagnostics import parse_service_types, validate_desired_state
from new_couchbase.common._utils import (timedelta_as_microseconds,
                                                ...
                                                validate_transcoder)
from new_couchbase.impl.server._utils import timedelta_as_timestamp
from new_couchbase.impl.server.durability import DurabilityParser
...
class ValidDiagnosticOptions:
    ...
    def get_valid_options(opt_type # type: OptionTypes
        ...
        valid_opts = copy.copy(ValidDiagnosticOptions._VALID_OPTS)
        if opt_type == OptionTypes.Ping:
            valid_opts.update({
                'report_id': {'report_id': validate_str},
                'service_types': {'service_types': parse_service_types},
            })
        elif opt_type == OptionTypes.Diagnostics:
            valid_opts.update({
                'report_id': {'report_id': validate_str},
            })
        elif opt_type == OptionTypes.WaitUntilReady:
            valid_opts.update({
                'desire_state': {'desire_state': validate_desired_state},
                'service_types': {'service_types': parse_service_types},
            })
        else:
            raise InvalidArgumentException(f"Invalid option type: {opt_type}")
