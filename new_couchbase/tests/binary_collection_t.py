...
from new_couchbase.exceptions import DocumentNotFoundException, InvalidArgumentException
from new_couchbase.options import (DecrementOptions,
                                   DeltaValue,
                                   IncrementOptions,
                                   SignedInt64)
from new_couchbase.result import CounterResult, MutationResult
...
class ClassicBinaryCollectionTests(BinaryCollectionTestSuite):
    ...
    def manifest_validated(self):
        ...
        test_list = set(BinaryCollectionTestSuite.TEST_MANIFEST).symmetric_difference(method_list)
        if test_list:
...
class ProtostellarBinaryCollectionTests(BinaryCollectionTestSuite):
    ...
    def manifest_validated(self):
        def valid_test_method(meth):
            attr = getattr(ProtostellarBinaryCollectionTests, meth)
            return callable(attr) and not meth.startswith('__') and meth.startswith('test')
        method_list = [meth for meth in dir(ProtostellarBinaryCollectionTests) if valid_test_method(meth)]
        test_list = set(BinaryCollectionTestSuite.TEST_MANIFEST).symmetric_difference(method_list)
        if test_list:
            pytest.fail(f'Test manifest invalid.  Missing/extra tests: {test_list}.')
