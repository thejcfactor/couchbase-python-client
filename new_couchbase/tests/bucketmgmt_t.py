...
from new_couchbase.management.buckets import (BucketSettings,
                                              BucketType,
...
class ProtostellarBucketManagementTests(BucketManagementTestSuite):
    SKIP_LIST = {
        'test_bucket_backend_default': '',
        'test_bucket_backend_ephemeral': '',
        'test_bucket_backend_magma': '',
    ...
    def can_run_test(self, request):
        ...
        if test_name in ProtostellarBucketManagementTests.SKIP_LIST:
            reason = ProtostellarBucketManagementTests.SKIP_LIST[test_name]
            pytest.skip(f'Skipping {test_name} due to : {reason}')

    @pytest.fixture(scope='class')
    def test_manifest_validated(self):
        def valid_test_method(meth):
            attr = getattr(ProtostellarBucketManagementTests, meth)
            return callable(attr) and not meth.startswith('__') and meth.startswith('test')
        method_list = [meth for meth in dir(ProtostellarBucketManagementTests) if valid_test_method(meth)]
        compare = set(BucketManagementTestSuite.TEST_MANIFEST).difference(method_list)
        return compare
