...
class CollectionTestSuite:
    ...
    def test_upsert_preserve_expiry_not_used(self, cb_env):
        key, value = cb_env.get_existing_doc()
        _, value1 = cb_env.get_new_doc()
        cb_env.collection.upsert(key, value, UpsertOptions(expiry=timedelta(seconds=30)))
        expiry_path = '$document.exptime'
        res = TestEnvironment.try_n_times(10,
                                          3,
                                          cb_env.collection.lookup_in,
...
class ClassicCollectionTests(CollectionTestSuite):
    ...
    def manifest_validated(self):
        ...
        test_list = set(CollectionTestSuite.TEST_MANIFEST).symmetric_difference(method_list)
        if test_list:
...
class ProtostellarCollectionTests(CollectionTestSuite):
    ...
    def can_run_test(self, request):
        ...
        if test_name in ProtostellarCollectionTests.SKIP_LIST:
            reason = ProtostellarCollectionTests.SKIP_LIST[test_name]
