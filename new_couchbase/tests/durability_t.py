...
from new_couchbase.durability import (ClientDurability,
                                  ...
                                  ReplicateTo,
                                  ServerDurability)
from new_couchbase.exceptions import DocumentNotFoundException, DurabilityImpossibleException, FeatureUnavailableException
from new_couchbase.options import (InsertOptions,
                               MutateInOptions,
                               RemoveOptions,
                               ReplaceOptions,
                               UpsertOptions)
...
class ClassicDurabilityTests(DurabilityTestSuite):
    ...
    def test_manifest_validated(self):
        ...
        test_list = set(DurabilityTestSuite.TEST_MANIFEST).symmetric_difference(method_list)
        if test_list:
...
class ProtostellarDurabilityTests(DurabilityTestSuite):

    SKIP_LIST = {
        # 'test_client_durable_insert': '',
        # 'test_client_durable_insert_fail': '',
        # 'test_client_durable_insert_single_node': '',
        # 'test_client_durable_mutate_in': '',
        # 'test_client_durable_mutate_in_fail': '',
        # 'test_client_durable_remove': '',
        # 'test_client_durable_remove_fail': '',
        # 'test_client_durable_remove_single_node': '',
        # 'test_client_durable_replace': '',
        # 'test_client_durable_replace_fail': '',
        # 'test_client_durable_replace_single_node': '',
        # 'test_client_durable_upsert': '',
        # 'test_client_durable_upsert_fail': '',
        # 'test_client_durable_upsert_single_node': '',
        # 'test_client_persist_to_extended': '',
    ...
    def can_run_test(self, request):
        ...
        if test_name in ProtostellarDurabilityTests.SKIP_LIST:
            reason = ProtostellarDurabilityTests.SKIP_LIST[test_name]
    ...
    def test_manifest_validated(self):
        def valid_test_method(meth):
            attr = getattr(ProtostellarDurabilityTests, meth)
            return callable(attr) and not meth.startswith('__') and meth.startswith('test')
        method_list = [meth for meth in dir(ProtostellarDurabilityTests) if valid_test_method(meth)]
        test_list = set(DurabilityTestSuite.TEST_MANIFEST).symmetric_difference(method_list)
        if test_list:
            pytest.fail(f'Test manifest invalid.  Missing/extra tests: {test_list}.')
